...
export class ReportesService {
  ...
  generarReporte(infoReport: any) {
    ...
    const lista_problemas = infoReport.problemas.map(prob => [
      ...
      prob.expProb,
      prob.principios,
      infoReport.problemas.indexOf(prob) + 1
    ]);
    ...
    this._evidenciasService.getAllEvidencias(infoReport.evaInfo.idEvaluacion).subscribe(async (evid: any) => {
      this.evidencias = evid;
      let lista_evidenvias = [];
      this.evidencias.map(evid => lista_evidenvias.push({
        idEvid: this.evidencias.indexOf(evid) + 1,
